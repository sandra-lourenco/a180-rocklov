...
Quando('a {string} e {string} solicita a locação desse equipo') do |email, password|
...
Então('devo ver os links: {string} e {string} no pedido') do |button_accept, button_reject|
  ...
  expect(@dash_page.order_action(button_accept)).to be true
  expect(@dash_page.order_action(button_reject)).to be true
end
